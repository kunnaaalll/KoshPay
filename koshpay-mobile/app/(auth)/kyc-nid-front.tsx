...
export default function KYCNIDFrontScreen() {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        alert(
          data.error || "Failed to save Front ID photo, Please Try Again :("
        );
        return;
      }
      router.push({pathname:"/(auth)/kyc-nid-back", params: { doc_kind: docKind }});
    } catch (error) {
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={styles.content}>
        ...
        <Text style={[styles.subtitle, { color: theme.textSecondary }]}>
          Take a clear photo of the front side of your National ID card
        </Text>
